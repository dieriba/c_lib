        ...
        usize capacity, usize *alloc_size)
{
    ...
    if (d_overflow_check_add_usize(capacity, extra, &total_elems))
        return false;
    if (d_overflow_check_mul_usize(total_elems, container->elem_size,
            alloc_size))
        return false;
    return true;
}

static usize container_elt_len(const Container *container, usize i)
{
    return container->elem_size * i;
}

static void *container_elt_pos(const Container *container, usize i)
{
    return (char *)container->data + container_elt_len(container, i);
}
...
static void container_write_sentinel(Container *container)
{
    if (container->data == NULL || !container_has_zero_sentinel(container))
        return;
    memset(container_elt_pos(container, container->len), 0,
        container->elem_size);
}


Result container_init(Container *container, usize elem_size, usize capacity,
        DBits8 opts)
{
    usize alloc_size;
    ...
    container->opts = opts;
    if (!container_compute_new_alloc_size(container, container->capacity,
            &alloc_size))
        return ERROR;
...
Container *container_new_from(const Container *src)
{
    ...
    new_container->len = src->len;
    new_container->capacity = src->capacity;
    new_container->elem_size = src->elem_size;
    new_container->opts = src->opts;
    if (!container_compute_new_alloc_size(src, src->capacity, &alloc_size))
        goto error;
    new_container->data = malloc(alloc_size);
    if (new_container->data == NULL)
        goto error;
    size_to_cpy = src->len * src->elem_size;
    memcpy(new_container->data, src->data, size_to_cpy);
    container_write_sentinel(new_container);
    return new_container;

error:
    free(new_container->data);
    free(new_container);
    return NULL;
}
...
Result container_increase_capacity_if_needed(Container *container,
        usize nb_elem_to_copy)
{
    ...
    if (nb_elem_to_copy <= container_nb_available_elem_slot(container))
        return OK;
    if (d_overflow_check_add_usize(container->capacity, nb_elem_to_copy,
            &new_capacity))
        return ERROR;
    if (d_overflow_check_mul_usize(new_capacity, GROWTH_POLICY,
            &new_capacity))
        return ERROR;
    if (!container_compute_new_alloc_size(container, new_capacity,
            &alloc_size))
        return ERROR;
...
Result container_insert(Container *dst, usize dst_pos, const void *data,
        usize len)
{
    usize dst_len;
    usize byte_pos;
...
Result container_push(Container *container, const void *elem)
{
    ...
    if (container_increase_capacity_if_needed(container, 1) == ERROR)
        return ERROR;
    memcpy(container_elt_pos(container, container->len), elem,
        container->elem_size);
    container->len++;
    container_write_sentinel(container);
    return OK;
...
Result container_pop(Container *container, void *out_elem)
{
    ...
    container->len--;
    if (out_elem != NULL)
        memcpy(out_elem, container_elt_pos(container, container->len),
            container->elem_size);
    container_write_sentinel(container);
    return OK;
}
...
Result container_swap_remove(Container *container, usize index, void *out_elem)
{
    if (container == NULL || index >= container->len)
        return ERROR;
    if (out_elem != NULL)
        memcpy(out_elem, container_elt_pos(container, index),
            container->elem_size);
    container->len--;
    if (index != container->len)
        memcpy(container_elt_pos(container, index),
            container_elt_pos(container, container->len),
            container->elem_size);
    container_write_sentinel(container);
    return OK;
}
