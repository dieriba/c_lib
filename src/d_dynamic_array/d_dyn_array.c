...
DynArray *d_dyn_array_append(DynArray *dyn_array, const void *data, usize nb_elem_to_copy)
{
	if (nb_elem_to_copy == 0 || (nb_elem_to_copy != 0 && data == NULL))
		return NULL;
	if (container_increase_capacity_if_needed(&dyn_array->data, &dyn_array->capacity, dyn_array->len, dyn_array->elem_size, nb_elem_to_copy) == ERROR)
		return NULL;
	memcpy(d_dyn_array_elt_pos(dyn_array, dyn_array->len), data, d_dyn_array_elt_len(dyn_array, nb_elem_to_copy));
	dyn_array->len += nb_elem_to_copy;
	return dyn_array;
}
...
DPointerArray *d_pointer_array_new(usize reserved_elem, bool null_terminated, DestroyElemFunc free_func)
{
	...
	array->pdata = malloc(sizeof(intptr_t *) * array->capacity);
	if (array->pdata == NULL)
	{
		free(array);
		return NULL;
...
DPointerArray *d_pointer_array_modify_capacity(DPointerArray *array, usize new_capacity)
{
	if (new_capacity <= array->capacity)
		return array;
	intptr_t *tmp = realloc(array->pdata, new_capacity * sizeof(intptr_t *));
	if (tmp == NULL)
		return NULL;
	array->pdata = tmp;
...
DPointerArray *d_pointer_array_push_back(DPointerArray *arr, const void *data)
{
	...
	if (container_increase_capacity_if_needed(&arr->pdata, &arr->capacity, arr->len, sizeof(void *), 1, null_terminated) == ERROR)
		return NULL;
	array->pdata[array->len++] = (intptr_t *)data;
	--array->capacity;
	if (null_terminated)
...
DPointerArray *d_pointer_array_remove_index_fast(DPointerArray *arr, usize index)
{
	...
	if (index >= array->len)
		return NULL;
	intptr_t *tmp = arr->pdata + (index * sizeof(intptr_t));
	(array->pdata + ) = array->pdata[array->len - 1];
	if (free_func != NULL)
		free_func(tmp);
