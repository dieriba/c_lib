#ifndef __d_dyn_string__H__
#define __d_dyn_string__H__
 ...
 *
 * Allocates and initializes a new _DynString structure. The newly created
 ...
 *
 * @return DynString* A pointer to the newly created _DynString structure. Returns
 ...
 *
 * Allocates and initializes a new _DynString structure based on the provided
 ...
 *
 * @return DynString* A pointer to the newly created _DynString structure containing
 ...
 *
 * Allocates and initializes a new `_DynString` structure that is a duplicate of the provided `_DynString`. The new string will contain the same
 * content as the original, including its null terminator. The new `_DynString` is independent of the original, meaning modifications to one will
 * not affect the other. The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure to be copied. The behavior is undefined if `dstring` is `NULL`.
 *
 * @return DynString* A pointer to the newly created `_DynString` structure containing a copy of the original string. Returns `NULL` if memory
 *         allocation.
 */
 ...
 *
 * Allocates and returns a new null-terminated C string containing a substring of the `_DynString` structure `dstring`.
 ...
 *
 * @param dstring A pointer to the `_DynString` structure from which to extract the substring. The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * Allocates and initializes a new _DynString structure with the specified initial
 ...
 *
 * @param reserve The initial capacity for the new _DynString, specifying the
 *                number of characters to reserve space for. The reserved space
 *                is pre-allocated to optimize performance for future string
 *                modifications.
 *
 * @return DynString* A pointer to the newly created _DynString structure with the
 ...
 *
 * Allocates and initializes a new _DynString structure based on a substring of
 ...
 * empty `_DynString`. If `pos + len` exceeds the string boundary, the substring will
 ...
 *            substring. If `str` is NULL, an empty `_DynString` will be created if
 *            `pos` is out of range.
 ...
 *
 * Adjusts the `_DynString` to contain a substring starting from the specified position `pos` and extending for `len` characters.
 * This operation changes the original `_DynString` to represent the new substring. If `pos` is greater than or equal to the length of
 ...
 * the existing `_DynString`. The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure to be modified. The behavior is undefined if `dstring` is `NULL`.
 * @param pos The starting index in the `_DynString` from which to begin the substring.
 ...
 *            include all characters from `pos` to the end of the string.
 *
 * @return DynString* A pointer to the modified `_DynString` structure containing the substring, or `NULL` if `pos` is greater than
 *         or equal to the length of the string, or if memory allocation fails.
 */
DynString *d_dyn_string_slice_in_place(DynString *dstring, usize pos, usize len);
 ...
 * the specified _DynString. The capacity indicates how many characters the
 * dynamic string can hold before needing to allocate additional memory.
 *
 * @param dstring A pointer to the _DynString structure whose capacity is to be
 *                retrieved. Must not be NULL. . The behavior is undefined if `dstring` is `NULL`.
 *
 * @return usize The current capacity of the _DynString, representing the number
 ...
 *
 * Adjusts the size of the given _DynString to the specified length. If the new
 ...
 * if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the _DynString structure to be resized. Must not be NULL.
 * 				  The behavior is undefined if `dstring` is `NULL`.
 * @param len The new length for the _DynString. If `len` is zero, the string will be
 *            resized to an empty string.
 *
 * @return DynString* A pointer to the resized _DynString structure. Returns `NULL` if
 ...
 *
 * Adjusts the capacity of the given _DynString to the specified new capacity. If the new capacity
 ...
 *
 * @param dstring A pointer to the _DynString structure whose capacity is to be modified.
 *                The behavior is undefined if `dstring` is `NULL`.
 * @param new_capacity The new capacity for the _DynString. If `new_capacity` is zero, the
 *                     function will set the internal capacity to zero, which may result in
 *                     an empty or truncated string.
 *
 * @return DynString* A pointer to the modified _DynString structure. Returns `NULL` if
 *         memory allocation fails.
 */
 ...
 *
 * Adds the specified character `c` to the end of the given _DynString. If necessary, additional
 ...
 *
 * @param dstring A pointer to the _DynString structure to which the character is to be appended.
 *                The behavior is undefined if `dstring` is `NULL`.
 * @param c The character to append to the end of the dynamic string.
 *
 * @return DynString* A pointer to the modified _DynString structure with the appended character.
 ...
 * to the end of the specified _DynString. If necessary, additional memory will be allocated to accommodate
 ...
 *
 * @param dstring A pointer to the _DynString structure to which the substring is to be appended.
 *                The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * @return DynString* A pointer to the modified _DynString structure with the appended substring.
 ...
 *
 * Concatenates the specified C string to the end of the given _DynString. The function
 ...
 *
 * @param dstring A pointer to the _DynString structure to which the C string is to be
 *                appended. Must not be NULL.
 * @param str_to_append A pointer to the null-terminated C string to append to the
 *                      _DynString. If str_to_append is NULL, no action is taken.
 *
 * @return DynString* A pointer to the updated _DynString structure with the C string
 ...
 *
 * Modifies the `_DynString` structure `dstring1` by appending the contents of `dstring2` to it. This operation will extend `dstring1` to include all characters from `dstring2`, including its null terminator if `dstring2` has one.
 *
 * @param dstring1 A pointer to the `_DynString` structure to which `dstring2` will be appended. Must not be `NULL`.
 * @param dstring2 A pointer to the `_DynString` structure whose contents will be appended to `dstring1`. Must not be `NULL`.
 *
 * @return DynString* A pointer to the modified `dstring1` after appending the contents of `dstring2`. Returns `NULL` if memory allocation fails.
 *
 ...
 *
 * Replaces the content of the `_DynString` pointed to by `dstring` with the content of `str`.
 * The existing `_DynString` content will be replaced by the new string, which is `str`. If necessary,
 * memory will be reallocated to fit the new string. The internal character array of `_DynString` will
 * be null-terminated after replacement. If `dstring` is `NULL` or if the `_DynString` pointed to by `dstring`
 * is `NULL`, or if `str` is `NULL`, the behavior is undefined.
 *
 * @param dstring A pointer to a pointer to the `_DynString` structure to be replaced.
 *                The behavior is undefined if `dstring` is `NULL`.
 * @param str A pointer to the null-terminated C string that will replace the content of `_DynString`.
 *            The behavior is undefined if `str` is `NULL`.
 *
 * @return DynString* A pointer to the `_DynString` structure with its content replaced by `str`.
 *         Returns `NULL` if memory allocation fails.
 */
 ...
 *
 * Replaces the content of the `_DynString` pointed to by `dstring` with the content of the `_DynString` pointed
 * to by `copy`. The existing content of the `_DynString` pointed to by `dstring` will be replaced by the content
 * of `copy`. If necessary, memory will be reallocated to accommodate the new content. The internal character
 * array of `_DynString` will always be null-terminated after replacement. If `dstring` or `copy` is `NULL`, or
 * if the `_DynString` pointed to by `dstring` is `NULL`, the behavior is undefined.
 *
 * @param dstring A pointer to a pointer to the `_DynString` structure to be replaced.
 *                The behavior is undefined if `dstring` is `NULL`.
 * @param copy A pointer to the `_DynString` whose content will be used to replace the content of the `_DynString`
 *             pointed to by `dstring`. The behavior is undefined if `copy` is `NULL`.
 *
 * @return DynString* A pointer to the `_DynString` structure with its content replaced by the content of `copy`.
 *         Returns `NULL` if memory allocation fails and sets *dstring to `NULL`.
 */
DynString *d_dyn_string_replace_from_dstring(DynString *dstring, const DynString *copy);

/**
 * @brief Compares two dynamic strings.
 *
 * Compares the content of the two `_DynString` structures `dstring1` and `dstring2` lexicographically, similar to the behavior of `strcmp`.
 ...
 *
 * @param dstring1 A pointer to the first `_DynString` structure to be compared. The behavior is undefined if `dstring1` is `NULL`.
 * @param dstring2 A pointer to the second `_DynString` structure to be compared. The behavior is undefined if `dstring2` is `NULL`.
 *
 ...
 *
 * This function returns the character located at the specified index `i` in the `_DynString`. If the index `i` is within the valid range
 ...
 *
 * @param dstring A pointer to the `_DynString` structure from which to retrieve the character.
 *                The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * Compares the content of the `_DynString` structure `dstring` with the null-terminated C string `c_str`. The comparison is done lexicographically,
 ...
 *
 * @param dstring A pointer to the `_DynString` structure to be compared. The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * This function determines whether the `_DynString` starts with the character `c`, considering an optional set of characters to ignore at the beginning
 ...
 *
 * @param dstring A pointer to the `_DynString` structure to be checked.
 *                The behavior is undefined if `dstring` is `NULL`.
 ...
 * @param ignore_set A string containing characters to ignore at the start of the `_DynString`. If `ignore_set` is `NULL`, no characters are ignored.
 *
 ...
 *
 * This function determines whether the `_DynString` ends with the character `c`, considering an optional set of characters to ignore at the end
 ...
 * @param ignore_set A string containing characters to ignore at the end of the `_DynString`. If `ignore_set` is `NULL`, no characters are ignored.
 *
 ...
 *
 * This function determines whether the `_DynString` starts with the substring `str`, considering an optional set of characters to ignore at the beginning
 * of the string. If the `ignore_set` is not `NULL`, the function will first skip over any characters at the start of the string that are found in the `ignore_set`.
 ...
 *
 * This function determines whether the `_DynString` ends with the substring `str`, considering an optional set of characters to ignore at the end
 * of the string. If the `ignore_set` is not `NULL`, the function will first skip over any characters at the end of the string that are found in the `ignore_set`.
 ...
 *
 * Searches for the first occurrence of the character `c` in the `_DynString`, starting from the specified position `pos` and moving
 ...
 * if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure in which to search.
 *                The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * @return usize The index of the first occurrence of `c` in `_DynString`, starting from `pos` and moving forward.
 ...
 *
 * Searches for the first occurrence of the specified character in the given _DynString.
 ...
 *
 * @param dstring A pointer to the _DynString structure in which to search for the character.
 *                Must not be NULL.
 * @param c The character to search for in the _DynString.
 *
 * @return usize The index of the first occurrence of the character in the _DynString.
 ...
 *
 * Searches for the first occurrence of a character in the `_DynString` that is not equal to the specified character `c`.
 ...
 *
 * @return usize The index of the first character in `_DynString` that is not equal to `c`.
 ...
 *
 * @return usize The index of the first character in `_DynString` that is not equal to `c`, starting from `pos`.
 ...
 *
 * Searches the `_DynString` structure `dstring` for the last occurrence of the character `c`, starting from the specified position `pos` and proceeding backwards to the beginning of the string.
 ...
 * The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure in which to search. The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * Searches for the last occurrence of the character `c` in the `_DynString`, starting from the end of the string and moving backwards
 ...
 * @param c The character to find.
 *
 * @return usize The index of the last occurrence of `c` in `_DynString`, starting from the end and moving backwards.
 *         Returns `MAX_SIZE_T` if the character is not found.
 */
 ...
 *
 * @return usize The index of the first character in `_DynString` that is not equal to `c`, starting from `pos` and searching backwards.
 *         Returns `MAX_SIZE_T` if no such character is found.
 */
 ...
 * @param c The character to compare against.
 *
 * @return usize The index of the first character in `_DynString` that is not equal to `c`, starting from the end of the string
 ...
 *
 * Searches for the first occurrence of the substring specified by `str` in the `_DynString`, starting from the specified position `pos`
 ...
 *            the function will return `MAX_SIZE_T`.
 *
 * @return usize The index of the first occurrence of the substring `str` in `_DynString`, starting from `pos` and moving forward.
 ...
 *
 * Searches for the first occurrence of the substring specified by `str` in the `_DynString`, starting from the beginning of the string
 ...
 *            The behavior is undefined if `str` is `NULL`.
 *
 * @return usize The index of the first occurrence of the substring `str` in `_DynString`, starting from the beginning and moving forward.
 ...
 *
 * Searches the `_DynString` structure `dstring` for the last occurrence of the substring `str`, starting from the specified position `pos` and proceeding backwards to the beginning of the string.
 ...
 *
 * Searches for the last occurrence of the substring specified by `str` in the `_DynString`, starting from the end of the string and moving backwards
 ...
 *            The behavior is undefined if `str` is `NULL`.
 *
 * @return usize The index of the last occurrence of the substring `str` in `_DynString`, starting from the end and moving backwards.
 *         Returns `MAX_SIZE_T` if the substring is not found.
 */
 ...
 *
 * Searches the `_DynString` structure `dstring` for the first occurrence of any character present in the null-terminated C string `set`,
 * starting from the specified position `pos`. If such a character is found, its index is returned. If none of the characters in `set`
 * are found after `pos`, or if `pos` is greater than or equal to the length of `dstring`, the function returns `MAX_SIZE_T`,
 * which is the maximum value representable by `size_t`. The behavior is undefined if `dstring` or `set` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure in which to search. The behavior is undefined if `dstring` is `NULL`.
 * @param set A pointer to a null-terminated C string containing the set of characters to search for. The behavior is undefined if `str` is `NULL`.
 ...
 *
 * @return usize The index of the first character in `dstring` that matches any character in `set` starting from `pos`.
 ...
 *
 * Searches the `_DynString` structure `dstring` for the first occurrence of any character present in the null-terminated C string `str`,
 ...
 *
 * Searches for the first occurrence of a character in the `_DynString` that is not in the set of characters specified by `str`,
 * starting from the given position `pos`. If such a character is found, its index is returned. If all characters from `pos`
 ...
 *            dynamic string, the function will return `MAX_SIZE_T`.
 *
 * @return usize The index of the first character in `_DynString` that is not in the set of characters specified by `str`,
 ...
 *
 * Searches the `_DynString` structure `dstring` for the first character that is not included in the null-terminated C string `str`.
 ...
 *
 * Searches the `_DynString` structure `dstring` for the last occurrence of any character present in the null-terminated C string `str`,
 ...
 *
 * Searches for the first occurrence of a character in the `_DynString` that is not present in the set of characters specified by `str`,
 ...
 *
 * Searches for the first occurrence of a character in the `_DynString` that satisfies the condition defined by the predicate function `fn`,
 ...
 *            the function will return `MAX_SIZE_T`.
 *
 * @return usize The index of the first character in `_DynString` that matches the predicate function `fn`, starting from `pos` and moving forward.
 *         Returns `MAX_SIZE_T` if no such character is found, if `pos` is out of range.
 */
 ...
 *            or zero if it does not match. The behavior is undefined if `fn` is `NULL`.
 *
 * @return usize The index of the first character in `_DynString` that matches the predicate function `fn`, starting from the beginning and moving forward.
 *         Returns `MAX_SIZE_T` if no such character is found.
 */
 ...
 *
 * Searches the `_DynString` structure `dstring` for the last character (in reverse order) that satisfies the condition defined by the predicate function `fn`.
 ...
 *
 * Searches for the last occurrence of a character in the `_DynString` that satisfies the condition defined by the predicate function `fn`,
 ...
 *            or zero if it does not match. The behavior is undefined if `fn` is `NULL`.
 *
 * @return usize The index of the last character in `_DynString` that matches the predicate function `fn`, starting from the end and moving backwards.
 *         Returns `MAX_SIZE_T` if no such character is found.
 */
 ...
 *
 * Searches the `_DynString` structure `dstring` for the first character that does not satisfy the condition defined by the predicate function `fn`.
 ...
 *
 * Searches the `_DynString` structure `dstring` for the first character (in reverse order) that does not satisfy the condition defined by the predicate function `fn`.
 ...
 *
 * Modifies the `_DynString` to remove all occurrences of the specified character `c` from the beginning of the string. This operation
 * adjusts the original `_DynString` to exclude the specified character from its start until a different character is encountered. If the
 * resulting string is empty after trimming, the `_DynString` will be modified to represent an empty string. The function does not allocate
 * new memory; it only adjusts the existing `_DynString`. The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure to be modified. The behavior is undefined if `dstring` is `NULL`.
 * @param c The character to be removed from the beginning of the string.
 *
 * @return DynString* A pointer to the modified `_DynString` structure with leading occurrences of the character `c` removed.
 */
 ...
 *
 * Allocates and initializes a new `_DynString` that represents a substring of the original `_DynString` with all occurrences of the specified
 ...
 * is empty after trimming, the new `_DynString` will represent an empty string. If the original `_DynString` is empty or if all characters
 * are equal to `c`, the resulting string will be empty. The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure from which to create the new trimmed string. The behavior is undefined if `dstring` is `NULL`.
 * @param c The character to be removed from the beginning of the string.
 *
 * @return DynString* A pointer to the newly created `_DynString` structure with leading occurrences of the character `c` removed.
 *         Returns `NULL` if memory allocation fails.
 */
 ...
 *
 * Modifies the `_DynString` to remove all characters from the beginning of the string that satisfy the condition defined by the predicate
 ...
 * string is empty after trimming, the `_DynString` will be modified to represent an empty string. The function does not allocate new memory;
 * it only adjusts the existing `_DynString`. The behavior is undefined if `dstring` or `fn` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure to be modified. The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * @return DynString* A pointer to the modified `_DynString` structure with leading characters removed based on the predicate function.
 */
 ...
 *
 * Allocates and initializes a new `_DynString` that represents a substring of the original `_DynString` with all characters removed from the
 ...
 * character that does not satisfy the predicate. If the resulting string is empty after trimming, the new `_DynString` will represent an
 ...
 *
 * @return DynString* A pointer to the newly created `_DynString` structure with leading characters removed based on the predicate function.
 *         Returns `NULL` if memory allocation fails.
 */
 ...
 *
 * Modifies the `_DynString` to remove all occurrences of the specified character `c` from the end of the string. This operation adjusts
 * the original `_DynString` to exclude the specified character from the end until a different character is encountered. If the resulting
 * string is empty after trimming, the `_DynString` will be modified to represent an empty string. The function does not allocate new memory;
 * it only adjusts the existing `_DynString`. The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure to be modified. The behavior is undefined if `dstring` is `NULL`.
 * @param c The character to be removed from the end of the string.
 *
 * @return DynString* A pointer to the modified `_DynString` structure with trailing occurrences of the character `c` removed.
 */
 ...
 * If the resulting string is empty after trimming, the new `_DynString` will represent an empty string. If the original `_DynString` is empty
 * or if all characters are equal to `c`, the resulting string will be empty. The behavior is undefined if `dstring` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure from which to create the new trimmed string. The behavior is undefined if `dstring` is `NULL`.
 * @param c The character to be removed from the end of the string.
 *
 * @return DynString* A pointer to the newly created `_DynString` structure with trailing occurrences of the character `c` removed.
 *         Returns `NULL` if memory allocation fails.
 */
 ...
 *
 * Modifies the `_DynString` to remove all characters from the end of the string that satisfy the condition defined by the predicate function `fn`.
 ...
 * the `_DynString` will be modified to represent an empty string. The function does not allocate new memory; it only adjusts the existing `_DynString`.
 * The behavior is undefined if `dstring` or `fn` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure to be modified. The behavior is undefined if `dstring` is `NULL`.
 * @param fn A pointer to a function that takes a character and returns a non-zero value if the character should be removed, or zero if it should be kept.
 *            The behavior is undefined if `fn` is `NULL`.
 *
 * @return DynString* A pointer to the modified `_DynString` structure with trailing characters removed based on the predicate function.
 */
 ...
 * that does not satisfy the predicate. If the resulting string is empty after trimming, the new `_DynString` will represent an empty string.
 * The behavior is undefined if `dstring` or `fn` is `NULL`.
 *
 * @param dstring A pointer to the `_DynString` structure from which to create the new trimmed string. The behavior is undefined if `dstring` is `NULL`.
 * @param fn A pointer to a function that takes a character and returns a non-zero value if the character should be removed, or zero if it should be kept.
 *            The behavior is undefined if `fn` is `NULL`.
 *
 * @return DynString* A pointer to the newly created `_DynString` structure with trailing characters removed based on the predicate function.
 *         Returns `NULL` if memory allocation fails.
 */
 ...
 *
 * Allocates and initializes a new `DArray` structure where each element of the array corresponds to a character from the given `_DynString` structure.
 * The conversion includes all characters from the `DynString`, including the null terminator if present.
 *
 * @param dstring A pointer to the `_DynString` structure to be converted. If `dstring` is `NULL`, the behavior is undefined.
 *
 ...
 */
// DArray*		d_dyn_string_convert_as_d_array(DynString* dstring);

/**
 * @brief Splits a dynamic string into substrings based on a set of delimiter characters.
 *
 * Splits the `_DynString` structure `dstring` into substrings using any character from the null-terminated C string `str` as delimiters.
 ...
 *
 * @param dstring A pointer to the `_DynString` structure to be split. The behavior is undefined if `dstring` is `NULL`.
 ...
 *
 * Splits the `_DynString` structure `dstring` into multiple substrings using the specified delimiter character `c`.
 ...
 *
 * Frees the memory associated with the `_DynString` structure pointed to by `dstring` and sets the pointer to `NULL`. This function ensures
 ...
 *
 * @param dstring A pointer to a pointer to the `_DynString` structure to be destroyed. If `*dstring` is `NULL`, the function has no effect.
