...
static DQueue *make_int_queue(usize capacity)
{
    DQueue *queue = NULL;

    D_TEST_EXPR(d_queue_new(&queue, capacity, sizeof(int)) == D_OK);
    D_TEST_NOT_NULL(queue);
    return queue;
}
...
static void test_new_creates_empty_queue(void)
{
    DQueue *queue = NULL;

    D_TEST_EXPR(d_queue_new(&queue, 0, sizeof(int)) == D_OK);
    D_TEST_NOT_NULL(queue);
    expect_size(queue, 0);
...
static void test_new_with_capacity_sets_capacity(void)
{
    ...
    D_TEST_EXPR(d_queue_new(&queue, 8, sizeof(int)) == D_OK);
    D_TEST_NOT_NULL(queue);
...
{
    D_TEST_EXPR(d_queue_new(NULL, 4, sizeof(int)) == D_ERR_INVALID_ARG);
}
...
static void test_new_rejects_zero_elem_size_and_leaves_output_null(void)
{
    ...
    D_TEST_EXPR(d_queue_new(&queue, 4, 0) == D_ERR_INVALID_ARG);
    D_TEST_NULL(queue);
}

static void test_push_single_value_updates_size_and_empty_flag(void)
{
    DQueue *queue = make_int_queue(1);
...
{
    DQueue *queue = make_int_queue(0);

    expect_capacity_at_least(queue, 0);
    push_int(queue, 7);
    expect_size(queue, 1);
...
static void test_queue_stores_pointer_values_fifo(void)
{
    ...
    D_TEST_EXPR(d_queue_new(&queue, 1, sizeof(char *)) == D_OK);
...
static void test_large_struct_elements_are_copied_and_fifo(void)
{
    ...
    D_TEST_EXPR(d_queue_new(&queue, 1, sizeof(LargeStruct)) == D_OK);
    for (int i = 0; i < 16; i++)
    {
...
static void test_binary_data_with_zero_bytes_round_trips(void)
{
    ...
    D_TEST_EXPR(d_queue_new(&queue, 1, sizeof(data)) == D_OK);
...
{
    DQueue *a = make_int_queue(1);
    DQueue *b = make_int_queue(1);
...
static void test_destroy_accepts_pointer_to_null_queue(void)
{
    DQueue *queue = NULL;
...
static void test_destroy_empty_queue_sets_pointer_to_null(void)
{
    DQueue *queue = make_int_queue(4);
...
static void test_destroy_non_empty_queue_sets_pointer_to_null(void)
{
    DQueue *queue = make_int_queue(2);
...
static void test_destroy_after_wraparound_sets_pointer_to_null(void)
{
    DQueue *queue = make_int_queue(4);
...
static void test_destroy_after_growth_sets_pointer_to_null(void)
{
    DQueue *queue = make_int_queue(1);
...
{
    DQueue *queue = NULL;
    char *owned_by_test = malloc(6);

    D_TEST_NOT_NULL(owned_by_test);
    memcpy(owned_by_test, "hello", 6);
    D_TEST_EXPR(d_queue_new(&queue, 1, sizeof(char *)) == D_OK);
    D_TEST_EXPR(d_queue_push(queue, &owned_by_test) == D_OK);

    d_queue_destroy(&queue);
    D_TEST_NULL(queue);
    D_TEST_STR_EQ(owned_by_test, "hello");
    free(owned_by_test);
}
...
{
    DTest tests[] = {
        D_TEST_GENERATE_TEST(test_new_creates_empty_queue),
        ...
        D_TEST_GENERATE_TEST(test_destroy_accepts_pointer_to_null_queue),
        D_TEST_GENERATE_TEST(test_destroy_empty_queue_sets_pointer_to_null),
        D_TEST_GENERATE_TEST(test_destroy_non_empty_queue_sets_pointer_to_null),
        D_TEST_GENERATE_TEST(test_destroy_after_wraparound_sets_pointer_to_null),
        D_TEST_GENERATE_TEST(test_destroy_after_growth_sets_pointer_to_null),
