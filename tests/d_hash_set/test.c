...
static void free_count_int_key(void *key, void *value)
{
    g_free_calls++;
    if (key != NULL)
        g_key_sum += *(int *)key;
...
static void free_count_big_key(void *key, void *value)
{
    const BigKey *k = (const BigKey *)key;
    g_free_calls++;
    if (k != NULL)
...
static void free_owned_string_key(void *key, void *value)
{
    char *s;
...
{
    D_TEST_EXPR(d_hash_set_new(NULL, sizeof(int), 0, hash_int_key, cmp_int_key, NULL) == D_ERR_INVALID_ARG);
}

static void test_new_rejects_zero_key_size(void)
{
    DHashSet *set = (DHashSet *)0x1;
    D_TEST_EXPR(d_hash_set_new(&set, 0, 0, hash_int_key, cmp_int_key, NULL) == D_ERR_INVALID_ARG);
    D_TEST_EXPR(set == (DHashSet *)0x1);
}

static void test_new_accepts_zero_capacity_and_aligns_capacity(void)
{
    DHashSet *set = new_int_set_custom(0, NULL);
    assert_size(set, 0);
...
static void test_destroy_null_set_is_noop(void)
{
    DHashSet *set = NULL;
    d_hash_set_destroy(&set);
    D_TEST_NULL(set);
...
{
    DHashSet *set = new_int_set_custom(1, NULL);
    for (int i = 0; i < 300; ++i)
        insert_int(set, i);
    assert_size(set, 300);
    for (int i = 0; i < 300; ++i)
...
{
    DHashSet *set = new_collision_set(0, NULL);
    for (int i = 0; i < 64; ++i)
        insert_int(set, i);
    assert_size(set, 64);
    for (int i = 0; i < 64; ++i)
...
{
    reset_counters();
    DHashSet *set = new_int_set_custom(0, free_count_int_key);
    int key = 10;
    int out = 0;
    insert_int(set, key);
    D_TEST_EXPR(d_hash_set_remove(set, &key, &out) == D_OK);
...
static void test_str_set_insert_and_lookup_with_distinct_pointer_same_text(void)
{
    ...
    D_TEST_EXPR(d_hash_set_new_str(&set, 0, NULL) == D_OK);
...
static void test_str_set_remove_transfers_owned_pointer(void)
{
    ...
    char *out = NULL;
    g_live_strings = 1;
    D_TEST_EXPR(d_hash_set_new_str(&set, 0, free_owned_string_key) == D_OK);
...
static void test_int32_wrapper_basic_usage(void)
{
    DHashSet *set = NULL;
    int32 key = 12345;
    D_TEST_EXPR(d_hash_set_new_int32_key(&set, 0, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &key) == D_OK);
    D_TEST_EXPR(d_hash_set_key_exists(set, &key) == true);
    assert_size(set, 1);
    d_hash_set_destroy(&set);
...
static void test_usize_wrapper_large_values(void)
{
    ...
    D_TEST_EXPR(d_hash_set_new_usize_key(&set, 0, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &a) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &b) == D_OK);
...
static void test_bool_wrapper_keeps_only_two_values(void)
{
    ...
    bool t = true;
    bool f = false;
    D_TEST_EXPR(d_hash_set_new_bool_key(&set, 0, NULL) == D_OK);
    for (int i = 0; i < 10; ++i)
    {
...
{
    DHashSet *set = NULL;
    D_TEST_EXPR(d_hash_set_new_char_key(&set, 0, NULL) == D_OK);
    for (int i = -64; i < 64; ++i)
    {
        char c = (char)i;
        D_TEST_EXPR(d_hash_set_insert(set, &c) == D_OK);
...
{
    DHashSet *set = NULL;
    D_TEST_EXPR(d_hash_set_new_u64_key(&set, 1, NULL) == D_OK);
    for (u64 i = 0; i < 500; ++i)
    {
...
static void test_binary_keys_with_embedded_zero_bytes(void)
{
    ...
    BinKey b = a;
    b.bytes[15] = 99;
    D_TEST_EXPR(d_hash_set_new(&set, sizeof(BinKey), 0, hash_bin_key, cmp_bin_key, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &a) == D_OK);
    D_TEST_EXPR(d_hash_set_key_exists(set, &a) == true);
...
{
    DHashSet *set = NULL;
    D_TEST_EXPR(d_hash_set_new(&set, sizeof(BinKey), 1, hash_bin_key, cmp_bin_key, NULL) == D_OK);
    for (int i = 0; i < 250; ++i)
    {
...
static void test_big_struct_key_fieldwise_after_remove(void)
{
    ...
    D_TEST_EXPR(d_hash_set_new(&set, sizeof(BigKey), 0, hash_big_key, cmp_big_key, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &key) == D_OK);
...
static void test_big_struct_destroyer_sees_all_live_keys(void)
{
    reset_counters();
    DHashSet *set = NULL;
    D_TEST_EXPR(d_hash_set_new(&set, sizeof(BigKey), 0, hash_big_key, cmp_big_key, free_count_big_key) == D_OK);
    for (int i = 1; i <= 20; ++i)
    {
...
static void test_pair_key_custom_comparator_distinguishes_fields(void)
{
    ...
    PairKey b = {2, 1};
    PairKey c = {1, 3};
    D_TEST_EXPR(d_hash_set_new(&set, sizeof(PairKey), 0, hash_pair_key, cmp_pair_key, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &a) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &b) == D_OK);
    D_TEST_EXPR(d_hash_set_key_exists(set, &a) == true);
    D_TEST_EXPR(d_hash_set_key_exists(set, &b) == true);
...
{
    DHashSet *set = NULL;
    D_TEST_EXPR(d_hash_set_new(&set, sizeof(int), 0, hash_int_two_groups, cmp_int_key, NULL) == D_OK);
    for (int i = 0; i < 160; ++i)
        insert_int(set, i);
...
{
    DHashSet *set = new_collision_set(1, NULL);
...
static void test_destroy_sets_pointer_to_null_after_large_table(void)
{
    DHashSet *set = new_int_set_custom(1, NULL);
...
{
    DHashSet *set = NULL;
    D_TEST_EXPR(d_hash_set_new_str(&set, 1, NULL) == D_OK);
    char *keys[120];
    for (int i = 0; i < 120; ++i)
    {
        char buf[32];
...
static void test_wrapper_remove_returns_key_for_u32(void)
{
    ...
    u32 key = 0xDEADBEEFu;
    u32 out = 0;
    D_TEST_EXPR(d_hash_set_new_u32_key(&set, 0, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &key) == D_OK);
    D_TEST_EXPR(d_hash_set_remove(set, &key, &out) == D_OK);
    D_TEST_EXPR(out == key);
    assert_size(set, 0);
...
static void test_wrapper_delete_for_int64_min_max(void)
{
    ...
    D_TEST_EXPR(d_hash_set_new_int64_key(&set, 0, NULL) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &a) == D_OK);
    D_TEST_EXPR(d_hash_set_insert(set, &b) == D_OK);
...
int main(void)
{
    DTest tests[] = {
        ...
        D_TEST_GENERATE_TEST(test_new_accepts_zero_capacity_and_aligns_capacity),
        ...
        D_TEST_GENERATE_TEST(test_destroy_null_set_is_noop),
        ...
        D_TEST_GENERATE_TEST(test_destroy_sets_pointer_to_null_after_large_table),
